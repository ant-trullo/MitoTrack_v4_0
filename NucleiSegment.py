...
class NucleiSegment:
    ...
    def __init__(self, frame1, circ_thr, lmp):
        ...
        circ  =  CircularityEstimate.CircularityEstimate(frame1.astype(np.int)).circ        # circularity of all the nuclei estimate
        ...
        lbl       =  skmr.label(frame1_sgm, connectivity=1).astype(np.uint16)                # labeling of all the correctly segmented nuclei
        left      =  (np.sign(frame1) - np.sign(frame1_sgm)).astype(np.bool)                # isolating the bad segmented nuclei
        left_lbl  =  skmr.label(left, connectivity=1).astype(np.uint16)                      # labeling the bad segmented nuclei
        left      =  np.sign(left_lbl).astype(np.bool)
        ...
        local_maxi    =  peak_local_max(distance, indices=False, footprint=np.ones((lmp, lmp), dtype=int), labels=skmr.label(left_er))
        local_mx      =  filters.gaussian(local_maxi, 1) > 0
        local_mx_lbl  =  skmr.label(local_mx, connectivity=1)
        ...
        lbl_fin  =  skmr.label(lbl_fin, connectivity=1).astype(np.uint16)        # relabeling of the new matrix

        left_left  =  skmr.label(np.sign(frame1) - np.sign(lbl_fin), connectivity=1).astype(np.uint16)      # if there are still left labels, they are add to final labeled matrix, eventual corrections will be left to the user for manual corrections

        lbl_fin  =  lbl_fin + (lbl_fin.max() + 1) * np.sign(left_left) + left_left
        lbl_fin  =  skmr.remove_small_objects(lbl_fin, 60).astype(np.uint16)

        lbl_fin  =  skmr.label(lbl_fin, connectivity=1).astype(np.uint16)
